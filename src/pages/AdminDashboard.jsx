...
const AdminDashboard = () => {
  const { bookings, updateBookingStatus, addOfflinePayment, changePassword, adminPassword } = useApp();
  ...
  const handlePasswordChange = (e) => {
    ...
    if (passwordState.new.length < 4) {
      setPasswordMsg({ text: 'Password must be at least 4 characters!', type: 'error' });
      return;
    }
        ...
        <div>
          ...
          <div className="flex gap-4 mt-2">
            <button 
              ...
              className={`text-sm font-bold pb-1 transition-all ${activeTab === 'Overview' ? 'text-primary border-b-2 border-primary' : 'text-text-muted hover:text-text'}`}
            >
              Overview
            </button>
            <button 
              onClick={() => setActiveTab('Settings')}
              className={`text-sm font-bold pb-1 transition-all ${activeTab === 'Settings' ? 'text-primary border-b-2 border-primary' : 'text-text-muted hover:text-text'}`}
            >
              Security Settings
            </button>
          </div>
          ...
          <div className="card">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted">Manage your administrative access credentials.</p>
              </div>
            </div>
            ...
            <form onSubmit={handlePasswordChange} className="space-y-6">
              ...
              {passwordMsg.text && (
                <div className={`p-4 rounded-xl text-sm font-bold flex items-center gap-2 ${passwordMsg.type === 'success' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                  <AlertCircle size={18} /> {passwordMsg.text}
                </div>
              )}

              <button type="submit" className="btn btn-primary px-8">Update Password</button>
            </form>
          </div>
        </motion.div>
      )}
